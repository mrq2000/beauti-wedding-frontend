...
const SignIn = () => {
  ...
  const redirectUrl = searchParams.get('redirect') || '/designer';
  const handleMouseDownPassword = (event: React.MouseEvent<HTMLButtonElement>) => {
    event.preventDefault();
  };

  const { isLoading, mutate: signIn } = useMutation(async (data: FormValues) => {
    const res = await api.post('/designers/sign-in', {
  ...
  return (
    <Box display="flex" alignItems="center" flexDirection="column" gap="16px">
      ...
      <Typography variant="caption">Quên mật khẩu? Vui lòng liên hệ admin để lấy lại mật khẩu</Typography>
    </Box>
  );
};
